...
/**
 * The persistent class for the MDOI_JPA_POLICY_MST_WF database table.
 * 
 */
@Entity
@Table(name = "MDOI_JPA_POLICY_MST_WF", schema = DoiJPAConstants.DOI_SCHEMA)
@NamedQuery(name = "DoiJPAMasterPolicyWF.findAll", query = "SELECT m FROM DoiJPAMasterPolicyWF m")
public class DoiJPAMasterPolicyWF extends BaseEntity {
	private static final long serialVersionUID = 1L;

	@Id
	@GeneratedValue(strategy = GenerationType.IDENTITY)
	@Column(name = "JPA_POLICY_MST_WF_ID", unique = true, nullable = false)
	private long jpaPolicyMstWfId;

	// bi-directional many-to-one association to MdoiJpaPolicyMst
	@ManyToOne

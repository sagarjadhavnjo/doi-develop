...
import gov.ifms.pvu.service.PVUSuspensionPayDetailsServiceImpl;

class PVUSuspensionPayDetailsServiceImplTest {
	@InjectMocks
	private PVUSuspensionPayDetailsServiceImpl  service;
	
	@Mock
	private PVUSuspensionPayDetailsRepository repository;
	
	@Mock
	private PVUSuspensionPayDetailsEntity entity;
	
	@Mock
	Page<PVUSuspensionPayDetailsEntity> page;
	
	@Mock
	List<PVUSuspensionPayDetailsEntity> lst;
	
	@Mock
	List<PVUSuspensionPayDetailsDto> list;
	
	@Mock
	PVUSuspensionPayDetailsConverter converter;
	@BeforeEach
	public void setup() {
	...
	@Test
	void testGetSuspensionPayDetails() throws CustomException {
		when(repository.getOne(1L)).thenReturn(entity);
		service.getSuspensionPayDetails(1l);
		verify(repository,times(1)).getOne(1l);
	}

	@Test
	void testSaveOrUpdateSuspensionPayDetails() throws CustomException {
		when(repository.save(entity)).thenReturn(entity);
		assertNotNull(service.saveOrUpdateSuspensionPayDetails(entity));
		verify(repository,times(1)).save(entity);
	}

	@SuppressWarnings("unchecked")
	@Test
	void testGetSuspensionPayDetailss() throws CustomException {
		 List<SearchParam> jsonArr = new ArrayList<SearchParam>();
	        ...
	        PagebleDTO<PVUSuspensionPayDetailsDto> data=service.getSuspensionPayDetailss(pageDetail);
		assertNotNull(data);
	}

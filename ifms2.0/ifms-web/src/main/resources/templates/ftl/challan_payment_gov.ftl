<body>
	<html>

	<head>
		<style>
		.text-center {
			text-align: center;
		}
		tr{
		line-height:1;
		}
		.table {
			width: 100%;
			border: none;
			border-color:#fbfbfb;
		}
		
		.table th {
			font-size: 8px;
		}
		
		.border_table th, .border_table td{
			border: 1px solid #dcd7d7;
		}
		
		.container {
			max-width: 1240px;
			margin: 0 auto;	
		}
		
		.header_report {
			padding: 10px 0;
			font-size: 9px;
			font-weight: 600;
		}
		
		.report_text td {
			font-size: 8px;
		}
		
		.header_label {
			font-size: 7px;
			font-weight: 600;
		}
		.pad-left5 {
			padding-left:5px;
		}
		.pad-left2 {
			padding-left:2px;
		}
		.align-right2 {
			text-align: right;
			padding-right:2px;
		}
		</style>
	</head>
	<div class="report container" style="border:1px solid black;">
		<table width="100%">
		<tr>
			<td style="width: 30%;">
			<img width="160px" height="50" style="vertical-align: middle;" src="data:image/png;base64,"></td>
			<td class=" header_report" style="width: 55%; padding-left: 50px;">GTR 6 Challan</td>
			<td style="width: 15%;"><img width="190" height="140"		src="data:image/png;base64,${qrCode}" /></td>
		</tr>
		<tr>
			<td colspan="3">
				<table width="100%" class="table report_text">
					<tr>
						<td><span class="header_label">GRN: </span> ${header.grnNo}</td>
						<td><span class="header_label">GRN Date : </span>${header.createdDate?string('dd-MMM-yyyy')}</td>
						<td><span class="header_label" >Expiry Date : </span> <#if header.expiryDate??> ${header.expiryDate?string('dd-MMM-yyyy')} </#if></td>
					</tr>
					<tr>
						<td><span class="header_label">CIN : </span><#if header.challanNumber??> </#if></td>
						<td><span class="header_label" colspan="1">CIN Date : </span>
							<#if header.challanDate??> </#if></td>
					</tr>
					<tr>
						
					</tr>
				</table>
			</td>
		</tr>
		<tr>
				<td colspan="3">
					<table width="100%" class="table">
						<tr>
							<td class="text-center header_report">Details of Taxpayer </td>
						</tr>
						<tr>
						<td>
							<table width="100%" class="table report_text">
									<tr>
												<td ><span class="header_label">Name : </span> <#if userDetailName??>  ${userDetailName}  </#if></td>
												<td ><span class="header_label" colspan="2">Email ID : </span><#if userDetailEmail??>  ${userDetailEmail}  </#if></td>											
												<td ><span class="header_label">Mobile No : </span><#if userDetailMob??>  ${userDetailMob}  </#if> </td>
									</tr>	
									<tr>
												<td ><span class="header_label">Registration No : </span></td>			
												<td ><span class="header_label">Tax Paid From Date : </span><#if header.fromDate??> ${header.fromDate?string('dd-MMM-yyyy')} </#if> </td>
												<td ><span class="header_label">To Date : </span><#if header.toDate??> ${header.toDate?string('dd-MMM-yyyy')}  </#if> </td>
												<td ><span class="header_label">Pin Code : </span></td>
									</tr>
									<tr>									
												<td colspan="4"><span class="header_label">Address : </span></td>
									</tr>
									</table>
									</td>
								</tr>
							</table>					
				</td>
			</tr>
			<tr>
				<td colspan="3">
					<table width="100%" class="table">
						<tr>
							<td class="text-center header_report">Office Details </td>
						</tr>
						<tr>
							<table width="100%" class="table report_text">
								<tr>
									<td ><span class="header_label">Name of Office : </span><#if officeName??>  ${officeName}</#if>  </td>
									<td > <span class="header_label">Challan Type : </span>${challanTypeName} </td>
									<td ><span class="header_label">Deposit A/c No : </span> <#if header.pdplaNo??>  ${header.pdplaNo}  </#if> </td>
								</tr>
								<tr>
									<td colspan="2"><span class="header_label">Treasury/PAO: </span><#if treasuryName??>  ${treasuryName}</#if> </td>
								</tr>
							</table>
						</tr>
					</table>
				</td>
			</tr>
			<tr>
				<td>
				
					<table width="100%" class="table">
						<tr>
							<td class="text-center header_report">Payment Details </td>
						</tr>
						<tr>
						<table width="100%" class="table report_text border_table"
							cellpadding="0" cellspacing="0" style="padding: 5px;">
							<tr>
								<th>Sr.No</th>
								<th>Purpose of Payment</th>
								<th>Head of Account</th>
								<th>Amount ( Rs. )</th>
							</tr>
							<#assign count = 0>
							<#foreach detail in details>
							<tr>
								<td class="pad-left2">  <#assign count = count + 1> ${count}</td>
								<td class="pad-left2">${detail.purpDesc}</td>
								<td class="pad-left2"><#if detail.majorHeadOfAccount??>${detail.majorHeadOfAccount}</#if></td>
								<td class="align-right2" >${detail.getFormattedPurpAmount()}</td>
							</tr>
							</#foreach>
							<tr>
								<td colspan="3" class="header_label align-right2" >Total Amount</td>
								<td class="header_label align-right2">${totalAmount}</td>
							</tr>
							<tr>
								<td colspan="4" style="text-align: left" class="header_label pad-left2">Total Amount (in
									words) : ${totalInWord}</td>
							</tr>
						</table>
					</tr>						
					</table>
				</td>
			</tr>
			<tr>
				<td>
					 <span class="header_label pad-left5">Payment Mode</span> <span style="font-size: 7px;">${paymentModeName}</span>						
				</td>
			</tr>
			<tr>
				<td>
				<#if ntfs = false ??>
					<table width="100%" class="table">
						<tr>
							<td class="text-center header_report header_label">If payment through NEFT / RTGS </td>
						</tr>
					</table>
					<table width="100%" class="table report_text">
						<tr>
							<td  ><span class="header_label">Remitting Bank:-</span> <#if bankName??>${bankName}</#if></td>
						</tr>	
						<tr>
							<td > <span class="header_label">Beneficiary Name:- </span>Government of Gujarat</td>
						</tr>
						<tr>
							<td ><span class="header_label">Beneficiary Account No.</span> </td>
						</tr>
						<tr>
							<td > <span class="header_label">Name of beneficiary bank </span>  Reserve Bank Of India</td>
						</tr>
						<tr>
							<td > <span class="header_label">Beneficiary Banks Indian Financial System Code ( IFSC ) :-</span></td>
						</tr>
						<tr>
							<td > <span class="header_label">Amount (Rs.) :-</span> ${totalAmount}</td>
						</tr>
						<tr>
						<td class="text-center header_label" style="font-size: 8px;" >Note :- Charges to be separately paid by the person making payment </td>
						</tr>
					</table>
				 </#if>	
				</td>
			</tr>
			
		</table>
	</div>

	</html>